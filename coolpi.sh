#!/bin/bash
# coolpi.sh — CoolPi v2.4.0: Self-Updating UI + update-coolpi + EEE Failsafe
...
# ————————————————
# 1) Quick-update mode: if called as update-coolpi, just update and exit
if [[ "$NAME" == "update-coolpi" ]]; then
  echo "Updating CoolPi (quick mode)…"
  if curl -fsSL "$REPO_URL" -o "$TMP_UP"; then
    NEW_VER=$(grep '^VERSION=' "$TMP_UP" | head -1 | cut -d'"' -f2)
    if [[ -n "$NEW_VER" && "$NEW_VER" != "$VERSION" ]]; then
      cp "$ME" "${ME}.eee.bak"
      mv "$TMP_UP" "$ME"
      chmod +x "$ME"
      echo "Updated to v$NEW_VER. Backup saved as $(basename "$ME").eee.bak"
    else
      echo "No new version (still v$VERSION)."
      rm -f "$TMP_UP"
    fi
  else
...
# ————————————————
# 3) Self-update on launch (version-based with EEE failsafe)
if curl -fsSL "$REPO_URL" -o "$TMP_UP"; then
  NEW_VER=$(grep '^VERSION=' "$TMP_UP" | head -1 | cut -d'"' -f2)
  if [[ -n "$NEW_VER" && "$NEW_VER" != "$VERSION" ]]; then
    echo "Update to v$NEW_VER found. Applying…"
    cp "$ME" "${ME}.eee.bak"
    mv "$TMP_UP" "$ME"
    chmod +x "$ME"
    exec "$ME" "$@"
...
=======================================
             COOLPI v${VERSION}              
 File Manager · App Store · Shell · Sys
=======================================
EOF
  sleep 1
}
...
show_storage(){
  ...
  sb=$(( 30-ub-fb )); bar=""
  for _ in $(seq 1 $ub); do bar+="\Z1█"; done
  for _ in $(seq 1 $sb); do bar+="\Z3█"; done
  for _ in $(seq 1 $fb); do bar+="\Z2█"; done
  dialog --colors --title "Storage" --msgbox \
"\nRoot FS Usage: $pct\n\n$bar\n\n\Z1Used\Z0 | \Z3System\Z0 | \Z2Free\Z0" 12 60
}

embedded_shell(){
  dialog --msgbox "Entering subshell.\nUse Ctrl+Shift+C/V to copy/paste." 6 60
  clear
  PS1="CoolPi> " bash --norc --noprofile
}

# ————————————————
# 6) File Manager Actions
...
github_publish(){
  ...
  if [[ "$http" =~ ^20 ]]; then
    dialog --msgbox "Published to $repo" 6 50
  else
    dialog --msgbox "GitHub Error: $http" 6 50
  fi
}
file_action(){
  dialog --menu "$(basename "$1")" 12 50 4 \
...
browse(){
  ...
  while true; do
    ...
    dialog --title "Browse: $dir" --menu "Select" 20 70 12 "${menu[@]}" 2>"$TMP_MENU" || break
    sel=$(<"$TMP_MENU")
    [[ "$sel" == "..BACK.." ]] && break
...
app_store_category(){
  case "$1" in
    Games)
      names=(Steam\ Upgrade SuperTux\ Pi \(0AD\) Scratch Terminal Colobot FreeCiv)
      urls=(
  ...
  menu=()
  for i in "${!names[@]}"; do
    fn="$APP_DIR/$(basename "${urls[i]}")"
    status=$([[ -f $fn ]] && echo "Installed" || echo "Install")
    menu+=("${names[i]}" "$status")
  done
  ...
  for i in "${!names[@]}"; do
    if [[ "${names[i]}" == "$sel" ]]; then
      fn="$APP_DIR/$(basename "${urls[i]}")"
      [[ -f $fn ]] && uninstall_app "$sel" || install_app "$sel" "${urls[i]}"
    fi
  done
}
...
# 8) App Launcher
launch_apps(){
  mapfile -t DESKTOPS < <(find /usr/share/applications ~/.local/share/applications -name '*.desktop' 2>/dev/null)
  menu=()
  ...
  cmd=$(<"$TMP_MENU"); "$cmd" &>/dev/null &
}

# ————————————————
# 9) System Utilities (with raspi-config)
system_utils(){
  while true; do
    dialog --menu "System Utilities" 16 60 6 \
      1 "Reboot" \
      2 "Shutdown" \
      3 "View Storage" \
      4 "Embedded Shell" \
...
main_menu(){
  while true; do
    dialog --title "CoolPi Home" --menu "Select an option" 15 60 6 \
      1 "Browse Files" \
      2 "Launch Apps" \
      3 "System Utilities" \
      4 "App Store" \
