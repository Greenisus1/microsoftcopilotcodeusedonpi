#!/bin/bash
# coolpi.sh — CoolPi v2.2: Self-Updating Terminal UI with Embedded Shell
...
file_action(){
  dialog --menu "File: $(basename "$1")" 12 50 4 \
    1 "Run" 2 "Publish" 3 "Delete" 4 "Back" 2>"$TMP"
  case $(<"$TMP") in
    1) run_file "$1";;
    2) github_publish "$1";;
    3) delete_file "$1";;
...
#### 8) App-Store Helpers
install_app(){
  curl -fsSL "$2" -o "$APP_DIR/$(basename "$2")"
...
app_store_category(){
  ...
  dialog --menu "$1 Apps" 16 60 6 "${menu[@]}" 2>"$TMP" || return
  sel=$(<"$TMP")
...
app_store(){
  dialog --menu "CoolPi App Store" 15 50 5 \
    1 "Games" \
    2 "System Utilities" \
    3 "Productivity" \
    4 "Development" \
...
main_menu(){
  while true; do
    dialog --title "CoolPi Home" --menu "Choose an option" 15 60 6 \
      1 "Browse Files" \
      2 "Launch Apps" \
      3 "System Utilities" \
      4 "App Store" \
